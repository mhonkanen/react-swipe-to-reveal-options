...
export default class SwipeItem extends Component {
	...
	revealLeft() {
		const { isActive, leftActionsWidth } = this.state;
		if (isActive) return;
	...
	revealRight() {
		const { isActive, rightActionsWidth } = this.state;
		if (isActive) return;
	...
	handlePanEnd(event) {
		const { isActive, startLeft, leftActionsWidth, rightActionsWidth } = this.state;
		const { disabled, threshold } = this.props;
		...
		// reveal left actions
		if (this.startLeft === 0 && this.direction === 'ltr' && currentLeft >= this.threshold) return animateSlide(leftActionsWidth, oldLeft);
	...
	render() {
		const { disabled, onSingleTap, onDoubleTap } = this.props;
		return (
			<Hammer

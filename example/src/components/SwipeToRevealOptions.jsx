...
export default class SwipeToRevealOptions extends Component {
	...
	handleSwipingRight(event, delta) {
		...
		let action = null;
		console.log(
			`handleSwipingRight: delta: ${delta}, visibilityThreshold: ${visibilityThreshold}, actionThreshold: ${actionThreshold}`
		);
		if (delta > visibilityThreshold) {
			action = 'leftVisible';
		}
	...
	shouldAbort(direction) {
		...
		const {
			leftOptions: { length: leftLength },
			rightOptions: { length: rightLength },
			callActionWhenSwipingFarRight,
			callActionWhenSwipingFarLeft
		} = this.props;
		if (transitionBack) {
		...
		if (direction === 'right') {
			return (!leftLength && !showRightButtons) || (showLeftButtons && !callActionWhenSwipingFarRight);
		}

		return (!rightLength && !showLeftButtons) || (showRightButtons && !callActionWhenSwipingFarLeft);
	}
